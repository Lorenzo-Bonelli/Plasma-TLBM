...
class LBmethod{
    ...
    //Constructor:
    public:
    LBmethod(const unsigned int NSTEPS, const unsigned int NX, const double u_lid, const double Re, const double rho0)
        : NSTEPS(NSTEPS), NX(NX), u_lid(u_lid), Re(Re), rho0(rho0) {}
    //Methods:
    void Initialize(){
...
int main(int argc, char* argv[]){
    ...
    LBmethod lb(NSTEPS,NX,u_lid,Re,rho);
    lb.Initialize();
    lb.Run_simulation();
