...
// Constructor
LBmethod::LBmethod(const size_t NSTEPS, size_t NX, size_t NY, const double u_lid, const double Re, const size_t num_cores)
    : NSTEPS(NSTEPS), NX(NX), NY(NY), u_lid(u_lid), Re(Re), num_cores(num_cores), c_s(1.0/ std::sqrt(3.0)),
    ...
    {
      //Keep stability
      //double dt=1.0/(this->NY * c_s);
      nu=(u_lid * this->NY)/Re; 
      //tau = (3.0 * nu)/dt + 0.5; 
      tau = (3.0 * nu) + 0.5; 

      if (tau<0.5){
        std::cerr << "Warning: Unstable tau detected. Increasing NY to improve stability." << std::endl;
        //Increase NY
        this->NY= static_cast<size_t>((Re*0.1)/(3*u_lid));
        this->NX=this->NY;
        nu = (u_lid * this->NY) / Re;
        //dt=1.0/(this->NY * c_s);
        //tau = (3.0 * nu)/dt+ 0.5;
        tau = (3.0 * nu) + 0.5; 
        std::cout << "Final tau: " << tau << ", NY: " << this->NY <<  std::endl;

      }
      else if (tau>2.0){
        std::cerr << "Warning: Unstable tau detected. Reducing NY to improve stability." << std::endl;
        //Decrease NY
        this->NY = static_cast<size_t>((Re*1.5)/(3*u_lid));
        this->NX = this->NY;
        nu = (u_lid * this->NY) / Re;
        //dt=1.0/(this->NY * c_s);
        //tau = (3.0 * nu)/dt+ 0.5;
